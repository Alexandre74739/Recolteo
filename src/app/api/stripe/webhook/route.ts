...
async function handleUpcomingInvoice(invoice: Stripe.Invoice) {
  ...
  if (!asso.renewal_reminder_7_sent && daysUntilRenewal !== null && daysUntilRenewal <= 7) {
    ...
    await admin.from("association").update({ renewal_reminder_7_sent: true }).eq("id_association", asso.id_association);
    return;
  }
  ...
  if (!asso.renewal_reminder_30_sent) {
    ...
    await admin.from("association").update({ renewal_reminder_30_sent: true }).eq("id_association", asso.id_association);
  }
}
